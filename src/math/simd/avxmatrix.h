...
public:
    AvxMatrix();
    AvxMatrix(const double data[16]);
    AvxMatrix(const double data[4][4]);
    AvxMatrix(double _11, double _12, double _13, double _14,
    ...
    bool operator==(const AvxMatrix& m2) const;
    AvxMatrix operator*(const AvxMatrix& m2) const;

    bool IsIdentity() const;
    double GetDeterminant() const;

    AvxMatrix Transposed() const;
    AvxMatrix Inversed() const;
